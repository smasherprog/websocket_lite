...
namespace SL {
	namespace WS_LITE {
		...
		void Listen(std::shared_ptr<WSListener> listener) {
		
			auto socket = std::make_shared<boost::asio::ip::tcp::socket>(listener->io_service);
			listener->acceptor.async_accept(*socket, [listener, socket](const boost::system::error_code& ec)
			{
				if (!ec)
				{
		...
		void TLSListen(std::shared_ptr<WSListener> listener) {
			auto socket = std::make_shared<boost::asio::ssl::stream<boost::asio::ip::tcp::socket>>(listener->io_service, *listener->sslcontext);
			listener->acceptor.async_accept(socket->lowest_layer(), [listener, socket](const boost::system::error_code& ec)
			{
				if (!ec)
				{
